...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* --- DESKTOP NAV --- */}
      <nav
        className={`fixed top-8 left-1/2 -translate-x-1/2 z-50 hidden md:flex items-center gap-2 px-4 py-2 rounded-xl
      ...
      <nav
        ...
        flex md:hidden items-center justify-around gap-1 w-[95vw] max-w-md px-2 py-2 rounded-xl
        backdrop-blur-md border border-white/10 transition-all duration-500
        ${scrolled ? "bg-white/10 shadow-2xl" : "bg-white/5 shadow-md"}
        ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}
      `}
        ...
        ${
          active === link.href.replace("/", "")
            ...
            <a
              ...
              className={`flex items-center justify-center w-12 h-12 rounded-xl text-xl transition-all duration-300
            ${
              active === link.href.replace("#", "")
                ? "bg-white/20 text-white shadow-[0_0_10px_rgba(255,255,255,0.6)]"
